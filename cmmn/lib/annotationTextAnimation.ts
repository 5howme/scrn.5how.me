import type { AnnotationTextAnimation } from "@/lib/types";

export const TEXT_ANIMATION_DURATION_MS = 400;
...
export function getTextAnimationState(
	...
	switch (animation) {
		case "fade": {
			// Mirror the fade-in over the last TEXT_ANIMATION_DURATION_MS before endMs.
			const fadeOut =
				typeof annotation.endMs === "number" && annotation.endMs > annotation.startMs
					? easeOutCubic(clamp((annotation.endMs - currentTimeMs) / TEXT_ANIMATION_DURATION_MS))
					: 1;
			return {
				opacity: Math.min(eased, fadeOut),
				scale: 1,
				translateX: 0,
				translateY: 0,
				revealProgress: 1,
