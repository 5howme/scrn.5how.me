...
type BaseProps = {
	...
	/** Accept #RRGGBBTT (alpha) values: enables the alpha slider and 8-digit hex input. */
	allowAlpha?: boolean;
};
...
export default function ColorPicker(props: ColorPickerProps) {
	const { selectedColor, colorPalette, translations, onUpdateColor, allowAlpha = false } = props;
	...
	const handleColorInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		// Only push when it's a complete #RGB or #RRGGBB value (#RGBA/#RRGGBBTT when alpha is allowed).
		const isValidHexColor = allowAlpha
			...
			{colorMode === "wheel" && (
				<>
					...
					<Colorful
						...
						onChange={(color) => {
							onUpdateColor(allowAlpha ? color.hexa : color.hex);
						}}
						style={{
							borderRadius: "8px",
						}}
