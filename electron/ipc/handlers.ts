...
} from "../../cmmn/lib/recordingSession";
import type {
	CursorRecordingData,
...
import { startWebHandoff } from "../webHandoff";
...
async function startCursorRecording(recordingId?: number) {
	// Linux native capture is not wired in this build, so the upstream guard
	// against a second portal session is unnecessary here.
...
export function registerIpcHandlers(
	...
	async function requestScreenAccess() {
		...
		try {
			...
			// TCC prompt without opening Screen Recorder's source selector above it.
		...
		if (process.platform === "darwin" && !access.granted) {
			...
			const detail =
				access.status === "missing-helper"
					...
					: "Allow Screen Recorder under System Settings → Privacy & Security → Accessibility, then press record again to start the countdown.";
		...
		if (!access.granted) {
			if (process.platform === "darwin" && access.status !== "not-determined") {
				...
				const messageOptions = {
					...
					detail:
						"Allow Screen Recorder in macOS System Settings, then come back and choose a screen or window.",
				} satisfies Electron.MessageBoxOptions;
				const result =
					mainWin && !mainWin.isDestroyed()
						? await dialog.showMessageBox(mainWin, messageOptions)
	...
	ipcMain.handle("start-web-handoff", async (_, editorOrigin: string) => {
		const screenVideoPath = normalizeVideoSourcePath(
			currentRecordingSession?.screenVideoPath ?? currentVideoPath,
		);
		if (!screenVideoPath) {
			return { success: false, error: "no-video" };
		}
		let cursorRecordingData: unknown;
		let cursorTelemetry: unknown;
		try {
			cursorRecordingData = await readCursorRecordingFile(screenVideoPath);
			const telemetry = await readCursorTelemetryFile(screenVideoPath);
			cursorTelemetry = telemetry.success ? telemetry.samples : undefined;
		} catch {
			// hand off without cursor data
		}
		try {
			const { port, token } = await startWebHandoff({
				editorOrigin,
				screenVideoPath,
				webcamVideoPath:
					normalizeVideoSourcePath(currentRecordingSession?.webcamVideoPath) ?? undefined,
				cursorCaptureMode: currentRecordingSession?.cursorCaptureMode,
				cursorRecordingData,
				cursorTelemetry,
			});
			return { success: true, port, token };
		} catch (error) {
			console.error("Failed to start web handoff:", error);
			return { success: false, error: String(error) };
		}
	});
	...
	async function loadProjectFileFromPath(filePath: string): Promise<ProjectFileResult> {
		try {
			...
			let session: import("../../cmmn/lib/recordingSession").RecordingSession | null = null;
			try {
	...
	ipcMain.handle("save-settings-preset", async (_, preset: unknown) => {
		try {
			const dialogOptions = buildDialogOptions(
				{
					title: "Save Settings Preset",
					defaultPath: path.join(RECORDINGS_DIR, "openscreen-preset.json"),
					filters: [{ name: "JSON", extensions: ["json"] }],
					properties: ["createDirectory", "showOverwriteConfirmation"],
				},
				getMainWindow(),
			...
			await fs.writeFile(result.filePath, JSON.stringify(preset, null, 2), "utf-8");
			return { success: true, path: result.filePath };
		} catch (error) {
			console.error("Failed to save settings preset:", error);
			return { success: false, error: String(error) };
		}
	});

	ipcMain.handle("load-settings-preset", async () => {
		try {
			const dialogOptions = buildDialogOptions(
				{
			...
			return { success: true, path: result.filePaths[0], preset: JSON.parse(content) };
		} catch (error) {
			console.error("Failed to load settings preset:", error);
			return { success: false, error: String(error) };
		}
	});
